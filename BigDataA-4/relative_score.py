...
def get_relative_score(joined_rdd):
    author = joined_rdd[1][1][1]
    subreddit_score = float(joined_rdd[1][1][0])
    subreddit_average = joined_rdd[1][0]
...
def main(inputs, output):
    ...
    reducing = line.map(getAvg).filter(lambda i: i[1] >= 0) # check if average > 0
    commentbysub = text.map(lambda c: (c['subreddit'], (c['score'], c['author'])))
    average_joinedTo_commentbysub = reducing.join(commentbysub)
    relative_RDD = average_joinedTo_commentbysub.map(get_relative_score).sortBy(lambda i: i[1], ascending=False) # to get relative scores and sort by keys in accending order
    relative_RDD.map(json.dumps).saveAsTextFile(output)
